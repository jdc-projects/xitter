...
async function createApp(dbOverride?: object): Promise<NestFastifyApplication> {
  ...
  app.setGlobalPrefix('api/media', { exclude: ['healthz', 'readyz'] });
  await app.init();
  return app;
}
...
describe('health probe wiring', () => {
  ...
  it('serves liveness at the root, outside the api/media prefix', async () => {
    app = await createApp();
  ...
  it('does not route probes under the api/media prefix', async () => {
    app = await createApp();

    const res = await app.inject({ method: 'GET', url: '/api/media/healthz' });
