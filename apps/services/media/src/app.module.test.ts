...
async function createApp(dbOverride?: object): Promise<NestFastifyApplication> {
  ...
  app.setGlobalPrefix('api/media/v1', { exclude: ['healthz', 'readyz'] });
  await app.init();
  return app;
}
...
describe('AppModule health wiring', () => {
  ...
  it('serves liveness at the root, outside the api/media/v1 prefix', async () => {
    app = await createApp();
  ...
  it('does not route probes under the api/media/v1 prefix', async () => {
    app = await createApp();

    const res = await app.inject({ method: 'GET', url: '/api/media/v1/healthz' });
