...
import { POST_TEXT_MAX, postSchema, usernameSchema } from "./schemas/domain.js";
import { createPostRequestSchema } from "./schemas/http.js";
...
describe("postSchema", () => {
  ...
  it("rejects text over the product limit", () => {
    expect(postSchema.safeParse({ ...basePost, text: "x".repeat(POST_TEXT_MAX + 1) }).success).toBe(false);
  });
