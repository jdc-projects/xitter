...
| Worker quiesce | minScale 0 on the worker Knative Services (Kubernetes API, job RBAC), restored even on failure | Volume reset: the stack is down (inherently quiesced); live reset warns when worker ports answer                                                 |
...
| Kafka                       | Reset consumer groups `xitter-fanout-worker`, `xitter-media-process-worker`, `xitter-search-index-worker` so workers resume from the new epoch (retained messages are not replayed) | Consumer-group reset to the log end                                                  |
