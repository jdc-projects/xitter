...
| `tofu-apply`     | `tofu apply` on `infra/iac/environments/shared` (Sentry — always converges first; prod's plan reads its DSN output, [ADR 0013](../../decisions/0013-shared-sentry-root.md)) then `infra/iac/environments/prod` with `-var image_tag=vX.Y.Z`. Dry runs plan both instead (detailed exit code 2 = changes = pass) and upload the prod plan. Self-hosted runner — the cluster API is private. |
...
| Sentry           | DSN from the shared root's state output (`xitter-shared`, [ADR 0013](../../decisions/0013-shared-sentry-root.md)) | same shared root — no dev-apply dependency; events tagged `environment=prod`                                                      |
