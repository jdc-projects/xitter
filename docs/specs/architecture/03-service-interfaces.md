...
| Method | Path                               | Auth | Description                                                                                                                              | Key fields                                       |
| ------ | ---------------------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| POST   | `/v1/posts`                        | user | Create post. `text` required (1–512 chars); `mediaIds` optional (max 4, uuid shape-only until #6 adds media validation); `replyToId` optional — rejected 403 when a block exists between replier and parent author (either direction), 404 when the parent is missing/deleted | req `text`, `mediaIds?`, `replyToId?` → res post |
...
Post payloads carry a counts read-model (`replies/likes/reposts`, zero-initialised; replies maintained by posts, likes/reposts by #8) and `media: []` until #6 hydrates attachments. Authors are not embedded in posts responses - callers join profiles through social (the web does this until #7's server-side joins).
