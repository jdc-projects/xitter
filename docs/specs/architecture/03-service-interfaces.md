...
| Auth            | `user` = Bearer user access token (demo realm, issued to the `web` client). `service` = client-credentials service token whose audience is the receiving service's client id (`svc-social`, `svc-posts`, `svc-media`, `svc-feed`, `svc-search`); machine callers are the five services plus `svc-worker-fanout`, `svc-worker-media-process`, `svc-worker-search-index`, `svc-reset`, and `svc-admin`, each scoped to exactly the internal APIs it calls (registry `CLIENT_AUDIENCES` in `packages/auth/src/clients.ts`, minted by `packages/scripts/src/keycloak.ts` and Tofu's keycloak.tf with parity enforced by `packages/scripts/src/keycloak-parity.test.ts`). In-cluster the edge validates user tokens and injects identity headers; locally services validate Bearer tokens directly — see [07-security.md](07-security.md). |
...
| Aspect           | Contract                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| URL              | `wss://{host}/api/feed/v1/ws?token={accessToken}` (token as query param; the feed service itself re-validates the token at upgrade - the edge cannot validate ws upgrades, so the service is the auth boundary). The web brokers that token at `GET /api/ws/feed-token` — under `/api/ws/` because the edge routes `/api/{service}/*` to services, and a web route there would be shadowed |
...
| `POST /api/posts/internal/posts/by-author`                   | fanout worker        | Author timeline page for the follow backfill (workers hold service tokens, not user tokens) |
