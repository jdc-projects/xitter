...
| Auth            | `user` = Bearer user access token (demo realm). `service` = client-credentials service token whose audience is the receiving service's client id (`svc-social`, `svc-posts`, `svc-media`, `svc-feed`, `svc-search`). In-cluster the edge validates user tokens and injects identity headers; locally services validate Bearer tokens directly — see [07-security.md](07-security.md). |
...
| Method | Path                               | Auth | Description                                                                                                                              | Key fields                                       |
| ------ | ---------------------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| POST   | `/v1/posts`                        | user | Create post. `text` required (1–512 chars); `mediaIds` optional (max 4, validated against media: allowed `png/jpeg/webp/gif`, ≤5MB each) | req `text`, `mediaIds?`, `replyToId?` → res post |
| DELETE | `/v1/posts/:id`                    | user | Delete own post (author only)                                                                                                            | res `204`                                        |
| GET    | `/v1/posts/:id`                    | user | Fetch post (hydrated)                                                                                                                    | res post                                         |
| GET    | `/v1/users/:id/posts`              | user | Cursor list of author's posts, newest first                                                                                              | paginated posts                                  |
| GET    | `/v1/posts/:id/replies`            | user | Cursor list of replies to a post                                                                                                         | paginated posts                                  |
| POST   | `/v1/posts/:id/interactions`       | user | Interact: `{kind: like\|bookmark\|repost}`. Rejected if caller is blocked by the post author                                             | req `kind` → res interaction                     |
| DELETE | `/v1/posts/:id/interactions/:kind` | user | Remove own interaction                                                                                                                   | res `204`                                        |
| GET    | `/v1/bookmarks`                    | user | Cursor list of caller's bookmarked posts                                                                                                 | paginated posts                                  |
...
Allowed types `png/jpeg/webp/gif`, max 5MB per file, enforced at slot creation and re-checked at post attach (see posts).
...
Service-token only (audience = receiving service client id). Never edge-exposed beyond the namespace; see [07-security.md](07-security.md).
