...
**Implementation — `@xitter/auth-nest`:** every service registers the global `AuthGuard` (Bearer user tokens, azp = `web`; `@Public()` opt-out for health endpoints) and the spec-shaped error-envelope filter. `internal/*` routes add `InternalGuard` (client-credentials tokens only: signature + issuer + audience = the service's own `svc-*` client id + azp allowlist covering `svc-*`, `svc-worker-*`, `svc-reset`; edge headers are never trusted there). Requests carrying `X-Access-Token` (edge-forwarded) are always re-validated.
