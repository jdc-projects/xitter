...
| Login bot protection | Cap.js captcha on login; site verification performed server-side in web app routes (`/api/auth/start`) **before** the OIDC redirect. Locally disabled via `XITTER_CAP_ENABLED=false` (the hosted Cap instance's CORS allowlist covers the deployed origin only); when enabled, all four `XITTER_CAP_*` vars (site/verify URLs + keys) must be set or the web app fails fast on first use (first request/render throws — deliberate: config errors should be loud, not silent captcha-off); the verify path is unit-tested with mocks and enabled wherever the widget origin is allowed. |
| Brute-force defence  | Keycloak realm-level brute-force protection on `xitter-demo` (Brute Force Mode: temporary lockout — Keycloak 26 `security_defenses.brute_force_detection` with defaults) — the login defence now that the realm is globally reachable; repeated failed logins temporarily lock the account.                                                                                                                                                                                                                                                                                             |
| CMS / admin          | Homelab **primary** realm; gated on `app-admin` (cms) and `system-admin` (admin) roles                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Service-to-service   | Keycloak client credentials; clients `svc-social`, `svc-posts`, `svc-media`, `svc-feed`, `svc-search`; plus machine clients `svc-worker-fanout` (aud: social, posts, feed), `svc-worker-media-process` (aud: media), `svc-worker-search-index` (aud: search, social), `svc-reset` (aud: all services). Token audience = receiving service's client id.                                                                                                                                                                                                                                  |
...
**Implementation — `@xitter/auth-nest`:** every service registers the global `AuthGuard` (Bearer user tokens, azp = `web`; the infrastructure probe routes `/healthz` + `/readyz` — prefix-excluded, never edge-exposed — are exempted by exact URL inside the guard, since the shared health controller cannot take a dependency on the auth decorators) and the spec-shaped error-envelope filter. `@Internal()` switches a route to client-credentials validation (signature + issuer + audience = the service's own `svc-*` client id + azp allowlist covering `svc-*`, `svc-worker-*`, `svc-reset`; edge headers are never trusted there) — user tokens are rejected even when their signature is valid. Requests carrying `X-Access-Token` (edge-forwarded) are always re-validated.
...
**Edge geo posture (T14):** the homelab edge geo-blocks non-UK traffic host-wide on `*.jd-chapman.dev`. xitter opts out deliberately: all `xitter-*` host routes and three paths on `idp.jd-chapman.dev` (`realms/xitter-demo`, `resources`, `js` — the demo login flow) run without the geoblock so the demo is globally reachable; cloudflare + crowdsec stay on, `/cms` + `/admin` rely on oidc-interactive auth, and everything else on the idp host (primary realm, Keycloak admin console) stays UK-only. Geo was never an auth control for the demo — brute-force protection on the realm is the login defence.
