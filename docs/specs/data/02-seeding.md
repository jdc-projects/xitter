...
| Failure      | Fail loudly and non-destructively; partial seeds are retried wholesale on the next run/reset. The seed's service-API calls retry transient failures (the CMS content phase does not yet - see the follow-up ticket) (502/503/504, connection refused) up to 3 times on a 2s backoff first (#82) — deploy pod-churn during the seed window is ridden out, not fatal |
