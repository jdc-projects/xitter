...
The corpus itself is pure (`packages/scripts/src/corpus.ts`) — no I/O, no clocks — and unit-tested for volumes, graph shape and cross-run fingerprint equality, so every environment derives identical data from the same constants. Image posts carry descriptive alt text (#133); the fingerprint is content-derived, so adding (or changing) corpus alt text intentionally changes it — the fingerprint compares runs of the _same_ corpus build, not across corpus edits.
