...
- **Viewport matrix (#151)**: the core journey specs (`nav`, `feed-flow`, `post-flow`, `profile`, `search-flow`) re-run in `mobile` (iPhone 13, 390×844) and `mobile-se` (iPhone SE, 375×667) projects; `mobile.spec.ts` (mobile-only) holds the horizontal-overflow guard — `document.documentElement.scrollWidth` must not exceed the viewport — plus the fluid-search-box and stacked-profile-header guarantees. The axe set re-runs at iPhone 13 in `a11y-mobile` (where WCAG 2.5.8 target-size bites); the desktop-first admin panel is scanned by the desktop `a11y` project only (`admin-a11y.spec.ts`).
