...
| Keycloak        | The env's own demo realm (`xitter-demo` dev, `xitter-demo-prod` prod — [ADR 0012](../decisions/0012-realm-per-environment.md)), `web` client, `svc-*` / `svc-worker-*` (+ `svc-reset` in dev) clients with per-audience mappers; creds into K8s Secrets                          |
| Workloads       | 11 `xitter-service` instances: 5 API services (+HPA), 3 Knative workers, web, cms, admin                                                                                                                                                                                         |
| Edge            | Homelab ingress module routes for `/`, `/api/{service}`, `/media`, `/cms`, `/admin`; geo-open (no geoblock middleware), plus `realms/<demo-realm>`, `resources`, `js` path routes on `idp.jd-chapman.dev` (demo login reachable globally; primary realm + `/admin` stay UK-only) |
