      ...
      nodePools = [
        {
          ...
          # live on dev: Initialized flipped with nodes-1 at 0/1, twice),
          # so with the operator's flow the voting config is always
          # frozen at {bootstrap, nodes-0} when the bootstrap is killed
          # and the cluster bricks (cluster-state APIs hang forever,
          # search wedges at boot). Overriding initial_master_nodes to
          # the pool pods - nodePool.env is appended AFTER the operator's
          # env (builders NewSTSForNodePool), and duplicate env entries
          # are last-wins - makes nodes-0/1 bootstrap each other: the
