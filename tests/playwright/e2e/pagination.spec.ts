 ...
 * demo5 is this suite's account.
 *
 * Search rides the exact same PaginatedPostList component + useCursorPages
 * hook (unit-tested in apps/web) fed by the same-shaped server action; the
 * profile tab exercises the shared control end-to-end here because its data
 * path is synchronous (no async index to wait on).
 */
